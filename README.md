...
erDiagram
    ...
    applications ||--o{ follow_ups : "has reminders"
    applications ||--o{ interview_rounds : "has rounds (v2)"
    applications ||--o{ application_tags : "tagged with (v2)"
    tags ||--o{ application_tags : "applied to"
    applications }o--o| resume_versions : "uses"
    applications }o--o| cover_letter_versions : "uses (v2)"
...
# Vercel Cron (set in Vercel dashboard)
...
> **⚠️ Important:** The schema includes Row-Level Security (RLS) policies. Do not skip these — without RLS, any authenticated user can read any other user's data.
...
> **Note:** The Vercel Hobby (free) tier allows one cron job per project, running once daily — this is exactly what the reminder check needs.
...
2. **Don't add features not in `AGENTS.md`** — if you want to propose a new feature, update `AGENTS.md` first (scope, data model, UI placement) and open a PR for discussion
3. **Branch naming**: `feature/<short-description>` or `fix/<short-description>`
4. **Commit messages**: short imperative summary (`Add status filter to dashboard`, not `added status filter`)
5. **One feature per PR** — don't bundle unrelated changes
6. **Test the Vercel preview URL** before merging
...
**Built with ☕ and frustration from losing track of 47 job applications in a Google Sheet.**
